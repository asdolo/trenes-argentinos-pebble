...
//como estoy usando sdk3 tengo que agregar la "status bar"
static StatusBarLayer *s_status_bar;

void draw_row_callback(GContext *ctx, Layer *cell_layer, MenuIndex *cell_index, void *callback_context)
{
	menu_cell_basic_draw(ctx, cell_layer, nombres_lineas[cell_index->row], NULL, NULL);
}

uint16_t num_rows_callback(MenuLayer *menu_layer, uint16_t section_index, void *callback_context)
{
	// Cantidad de lineas de trenes
	return CANT_LINEAS;
}

void select_click_callback(MenuLayer *menu_layer, MenuIndex *cell_index, void *callback_context)
{
	// Abro ventana de ramales
	dialog_window_ramales_push(cell_index->row);
}
